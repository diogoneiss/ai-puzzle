...
public class HillClimbing extends SearchAlgorithm {
    ...
    private PuzzleState pickRegularNeighbor(PuzzleState currentState, List<Grid> neighbors, List<Direction> directions) {
        ...
        // Create a list of neighbor PuzzleStates
        List<PuzzleState> neighborStates = new ArrayList<>();
        for (int i = 0; i < neighbors.size(); i++) {
            neighborStates.add(new PuzzleState(currentState, neighbors.get(i), directions.get(i)));
        }

        // Create a list of values for each PuzzleState
        List<Integer> values = neighborStates.stream()
        ...
        for (int i = 0; i < neighborStates.size(); i++) {
            if (values.get(i) <= bestDistance) {
                bestNeighbor = neighborStates.get(i);
                bestDistance = values.get(i);
            }
        }
